...
import { SHOWCASE_ACCENT, SHOWCASE_ACCENT_RGB } from "@/lib/theme-colors";
...
export function DiagnosticAnimation() {
  ...
  const t = (fn: () => void, s: number) =>
    timerRefs.current.push(setTimeout(fn, s * 1000));
  ...
  useEffect(() => {
    const run = () => {
      ...
      for (let i = 0; i <= targetName.length; i++) {
        t(
          () => setCompanyNameTyped(targetName.substring(0, i)),
          2.5 + i * 0.08,
        );
      }
  ...
  return (
    <div className="flex-1 w-full min-h-[320px] relative overflow-hidden grid place-items-center p-4">
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === "company" && (
            ...
            >
              <div className="flex flex-col gap-1.5">
                <span className="text-[10px] text-showcase-accent font-medium tracking-wider uppercase">
                  Company Name
                </span>
              ...
              <div className="flex flex-col gap-1.5 relative z-20">
                ...
                <div
                  className={`w-full h-10 bg-[#111111]/80 border ${companySizeState === "open" || companySizeState === "selected" ? "border-showcase-accent/50 ring-1 ring-showcase-accent/30" : "border-white/10"} rounded-md flex items-center justify-between px-3 relative transition-all duration-300`}
                >
                  <span
                    className={`text-sm ${companySizeState === "done" ? "text-white/90" : "text-white/40"}`}
                  >
                    {companySizeState === "done"
                      ? "50-200 Employees"
                      : "Select size..."}
                  </span>
                  <svg
                    className={`w-4 h-4 text-white/40 transition-transform duration-300 ${companySizeState === "open" || companySizeState === "selected" ? "rotate-180 text-showcase-accent" : ""}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                ...
                {/* Dropdown Menu */}
                <AnimatePresence>
                  {(companySizeState === "open" ||
                    companySizeState === "selected") && (
                    <motion.div
                          ...
                          <div
                            ...
                            className={`px-3 py-2 text-sm rounded-md transition-colors duration-200 ${
                              companySizeState === "selected" && i === 2
                                ? "bg-showcase-accent/15 text-showcase-accent"
                                : "text-white/60"
                            }`}
                          >
                            {opt}
          ...
          {phase === "form" && (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-[10px] sm:text-xs text-showcase-accent font-medium tracking-wider uppercase">
                  {currentStep.phase}
                </span>
              </div>
                ...
                {currentStep.options.map((opt, i) => (
                  <motion.div
                    ...
                    className={`flex items-center gap-3 rounded-lg px-4 py-3 border transition-colors duration-300 ${
                      selectedIdx === i
                        ? "bg-showcase-accent/10 border-showcase-accent/30 shadow-[0_0_10px_theme(colors.showcase-accent/10%)]"
                      ...
                      <AnimatePresence>
                        {selectedIdx === i && (
                          <motion.div
                            ...
                            animate={{ scale: 1 }}
                            exit={{ scale: 0 }}
                            className="w-2 h-2 bg-showcase-accent rounded-full"
                          />
                        )}
                      </AnimatePresence>
                    </div>
                    <span
                      className={`text-xs sm:text-sm ${selectedIdx === i ? "text-showcase-accent" : "text-white/75"}`}
                    >
                      {opt}
                    </span>
                  </motion.div>
          ...
          {phase === "thinking" && (
            ...
            >
              <div className="relative w-12 h-12 sm:w-16 sm:h-16 mb-2">
                {[
                  ...
                  <motion.div
                    ...
                    className="absolute w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 rounded-full bg-showcase-accent"
                    style={{
                      top: pos.top,
                      left: pos.left,
              ...
              <span className="text-sm sm:text-base text-showcase-accent font-medium tracking-wide">
        ...
        <div className="relative w-28 h-28 flex items-center justify-center mb-8">
          ...
          >
            ...
            <motion.circle
              ...
              r="42"
              fill="none"
              stroke={SHOWCASE_ACCENT}
              strokeWidth="6"
              strokeDasharray={264}
              initial={{ strokeDashoffset: 264 }}
              animate={{
                strokeDashoffset: barsVisible ? 264 - 264 * 0.42 : 264,
              }}
              transition={{ duration: 1.5, type: "spring", bounce: 0 }}
              className="drop-shadow-[0_0_8px_theme(colors.showcase-accent/40%)]"
            />
          </svg>
            ...
            <div key={dim.label} className="text-xs space-y-1.5">
              <div className="flex justify-between text-white/85 font-light">
                ...
                <motion.span
                  ...
                  animate={{ opacity: barsVisible ? 1 : 0 }}
                  transition={{ delay: dim.delay }}
                  className="text-showcase-accent"
                >
                  {dim.val}%
                </motion.span>
              </div>
              <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden relative">
                <motion.div
                  ...
                  className="absolute left-0 top-0 h-full bg-showcase-accent rounded-full"
                />
              </div>
            </div>
          ))}
      ...
      <motion.div
        animate={{
          opacity: phase === "improvements" ? 1 : 0,
          scale: phase === "improvements" ? 1 : 1.05,
        }}
        transition={fadeSpring}
          ...
          {[
            {
              ...
              desc: "Siloed data causing sync delays",
            },
            {
              title: "Workflow Inefficiency",
              desc: "High overhead to scale ops",
            },
          ].map((item, i) => (
            <motion.div
              key={i}
              ...
              animate={{
                opacity: phase === "improvements" ? 1 : 0,
                y: phase === "improvements" ? 0 : 15,
              }}
              transition={{
                delay: phase === "improvements" ? i * 0.2 : 0,
                type: "spring",
              }}
      ...
      <AnimatePresence>
        <motion.div
          ...
          animate={{
            opacity: phase === "document" ? 1 : 0,
            scale: phase === "document" ? 1 : 1.05,
          }}
        ...
        >
          {phase === "document" && (
            <div className="flex flex-col items-center gap-3 animate-fade-in-up">
              <div
                className="relative w-[150px] sm:w-[164px] rounded-xl overflow-hidden bg-gradient-to-b from-[#171b16] to-[#0c0e0b] border border-showcase-accent/25"
                style={{
                  boxShadow:
                    `0 14px 34px -10px rgba(0,0,0,0.75), 0 0 22px rgba(${SHOWCASE_ACCENT_RGB},0.14), inset 0 1px 0 rgba(255,255,255,0.06)`,
                  animation: "doc-float 3.2s ease-in-out infinite",
                }}
              >
                {/* top accent bar */}
                <div className="h-[3px] w-full bg-gradient-to-r from-showcase-accent/40 via-showcase-accent to-showcase-accent/40" />
                {/* header */}
                <div className="flex items-center gap-2 px-3 pt-2.5 pb-1.5">
                  <div className="w-6 h-6 rounded-md bg-showcase-accent/12 border border-showcase-accent/40 flex items-center justify-center text-showcase-accent shrink-0">
                    <svg
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="h-[6px] w-[64%] rounded-full bg-showcase-accent/70" />
                ...
                <div className="px-3 pb-3 pt-1 space-y-[7px]">
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <span className="text-[7px] leading-none font-semibold text-showcase-accent bg-showcase-accent/12 border border-showcase-accent/30 rounded px-1.5 py-[3px]">
                      42%
                    </span>
                    <div className="h-[4px] flex-1 rounded-full bg-white/10 overflow-hidden">
                      <div className="h-full w-[42%] rounded-full bg-showcase-accent/70" />
                    </div>
                  </div>
                </div>
                {/* scanning light */}
                <div
                  ...
                  style={{
                    background:
                      `linear-gradient(to bottom, transparent, rgba(${SHOWCASE_ACCENT_RGB},0.28), transparent)`,
                    animation: "doc-scan 2.4s ease-in-out infinite",
                  }}
                />
              </div>
              <span className="text-[10px] sm:text-[11px] text-showcase-accent/90 uppercase tracking-[0.22em] font-medium px-3 py-1 bg-[#0d0d0d]/80 border border-white/5 rounded-md whitespace-nowrap backdrop-blur-sm shadow-[0_4px_12px_rgba(0,0,0,0.5)]">
...
export function ConsoleAnimation() {
  ...
  return (
    <div className="w-full h-full flex flex-col justify-end pb-14 sm:pb-20 p-4 font-light relative">
      <div className="flex flex-col gap-3 w-full max-w-[100%] mx-auto mb-2">
        ...
        <motion.div
          initial={false}
          animate={{
            opacity: phase === "ai_typing" ? 1 : 0,
            y: phase === "ai_typing" ? 0 : 16,
          }}
          transition={{ type: "spring", stiffness: 300, damping: 24 }}
          ...
          <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl rounded-tl-sm px-4 py-2 sm:px-4 sm:py-2.5 shadow-[0_8px_32px_rgba(0,0,0,0.2)] flex items-center justify-center">
            <div className="relative w-4 h-4 opacity-80">
              {[
                ...
                <div
                  key={i}
                  className="absolute w-[3px] h-[3px] rounded-full bg-showcase-accent animate-octagon-dot"
                  style={{
                    top: pos.top,
                    left: pos.left,
        ...
        <motion.div
          initial={false}
          animate={{
            opacity: ["user_yes", "thinking", "response"].includes(phase)
              ? 1
              : 0,
            y: ["user_yes", "thinking", "response"].includes(phase) ? 0 : 16,
          }}
          transition={{ type: "spring", stiffness: 300, damping: 24 }}
        ...
        <motion.div
          initial={false}
          animate={{
            opacity: phase === "thinking" || phase === "response" ? 1 : 0,
            y: phase === "thinking" || phase === "response" ? 0 : 16,
          }}
          transition={{ type: "spring", stiffness: 300, damping: 24 }}
          className={`flex flex-col gap-3 ${phase === "thinking" || phase === "response" ? "" : "hidden"}`}
        >
          {phase === "thinking" && (
          ...
          {phase === "response" && (
            <div className="flex flex-col gap-2.5 pl-1 animate-fade-in-up mt-1">
              ...
              {[
                ...
                {
                  title: "Triage Flow",
                  desc: "12 bottlenecks detected in manual validation step.",
                  delay: "0.4s",
                },
                {
                  title: "Conversion",
                  desc: "Drop-off rate at stage 3 is 18% higher than benchmark.",
                  delay: "0.8s",
                },
              ].map((item, i) => (
                <div
                ...
                >
                  <div className="w-1.5 h-1.5 rounded-full bg-showcase-accent mt-1.5 shrink-0" />
                  <div className="flex flex-col">
                    <span className="text-white/80 text-[12px] sm:text-[13px] leading-snug">
                      {item.title}:{" "}
      ...
      <div className="absolute bottom-2 sm:bottom-4 left-4 right-4 bg-white/10 backdrop-blur-lg border border-white/20 rounded-full pl-3 pr-2 py-2 flex items-center gap-3 shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
        ...
        <div className="flex-1 text-[14px] truncate flex items-center transition-colors">
          {phase === "typing" ? (
            <span className="text-white/90">
              {typedText}
              <span className="ml-[2px] w-[2px] h-3.5 bg-white/60 animate-pulse inline-block align-middle" />
            </span>
          ) : (
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button className="w-8 h-8 rounded-full bg-showcase-accent flex items-center justify-center text-[#494949] hover:bg-[#7ac4c7] transition-colors">
...
export function WorkflowAnimation() {
  ...
  const fullText =
    "Create workflow to extract email leads and send them to Slack.";
  ...
  return (
    <div className="w-full flex flex-col gap-4 h-full justify-end relative pb-14 sm:pb-20 p-4 font-light">
      <div className="flex flex-col gap-3 sm:gap-4 w-full max-w-[100%] mx-auto mb-4">
        ...
        <div
          className={`flex items-center gap-2.5 transition-all duration-300 ${phase === "generating" || phase === "generated" || phase === "buttons" ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 hidden"}`}
        >
          {phase === "generating" ? (
            <>
              <div className="relative w-4 h-4 opacity-80 shrink-0">
        ...
        {(phase === "generated" || phase === "buttons") && (
          <SpotlightCard className="w-full p-4 sm:p-5 flex flex-col gap-4 sm:gap-5 relative shadow-2xl animate-fade-in-up">
            <div
              className="text-[9px] sm:text-[10px] text-white uppercase tracking-widest text-center font-light z-10"
              style={{ fontFamily: "'Doto', 'Courier New', monospace" }}
            >
              Workflow Generated
            </div>
            ...
            <div className="flex items-center justify-between w-full max-w-[400px] mx-auto z-10 relative">
              ...
              <div
                className="absolute top-1/2 left-[10%] right-[10%] h-[2px] -translate-y-1/2 -z-10 origin-left animate-scale-x rounded-full"
                style={{
                  background:
                    `linear-gradient(to right, rgba(${SHOWCASE_ACCENT_RGB},0.1), rgba(${SHOWCASE_ACCENT_RGB},0.8), rgba(193,204,200,0.1))`,
              ...
              {/* flowing data dots */}
              <span
                className="absolute top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-showcase-accent -z-10"
                style={{
                  boxShadow: `0 0 12px 2px ${SHOWCASE_ACCENT}`,
                  animation: "node-flow-continuous 1.2s ease-in-out infinite",
                }}
              />
              <span
                className="absolute top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-[#c1ccc8] -z-10"
                style={{
                  boxShadow: "0 0 12px 2px #c1ccc8",
                  animation: "node-flow-continuous 1.2s ease-in-out infinite",
                  animationDelay: "0.6s",
                }}
              />
              ...
              <div
                className="relative flex flex-col items-center w-[72px] sm:w-[100px] rounded-2xl pt-2 pb-2.5 px-2 bg-gradient-to-b from-[#23262b] to-[#15171b] border border-white/10 z-10"
                style={{
                  boxShadow:
                    "0 10px 24px -8px rgba(0,0,0,0.65), inset 0 1px 0 rgba(255,255,255,0.06)",
                  animation: "node-pop 0.6s ease-out both",
                }}
              >
                {/* output handle */}
                <span className="absolute top-1/2 -translate-y-1/2 -right-[6px] w-3 h-3 rounded-full bg-[#0c0d0f] border-2 border-showcase-accent z-20" />
                <div className="flex items-center gap-1 mb-1.5">
                  <span className="w-1 h-1 rounded-full bg-showcase-accent" />
                  <span className="text-[6.5px] sm:text-[8px] uppercase tracking-[0.14em] text-white/65 font-semibold">
                    Trigger
                  </span>
                </div>
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center bg-white shadow-[0_2px_6px_rgba(0,0,0,0.35)] mb-1.5">
                  <img
              ...
              <div
                className="relative flex flex-col items-center w-[72px] sm:w-[100px] rounded-2xl pt-2 pb-2.5 px-2 z-20 border border-showcase-accent/30 bg-gradient-to-b from-[#23262b] to-[#15171b]"
                style={{
                  boxShadow:
                    `0 10px 26px -8px rgba(0,0,0,0.7), 0 0 20px rgba(${SHOWCASE_ACCENT_RGB},0.16), inset 0 1px 0 rgba(255,255,255,0.06)`,
                  animation: "node-pop 0.6s ease-out 0.2s both",
                }}
              >
                {/* live indicator */}
                <span className="absolute top-[-4px] right-[-4px] w-3 h-3 rounded-full bg-showcase-accent premium-ping z-30" />
                <span className="absolute top-[-4px] right-[-4px] w-3 h-3 rounded-full bg-showcase-accent border border-[#0c0d0f] z-30" />
                {/* handles both sides */}
                <span className="absolute top-1/2 -translate-y-1/2 -left-[6px] w-3 h-3 rounded-full bg-[#0c0d0f] border-2 border-showcase-accent z-20" />
                <span className="absolute top-1/2 -translate-y-1/2 -right-[6px] w-3 h-3 rounded-full bg-[#0c0d0f] border-2 border-[#c1ccc8] z-20" />
                <div className="flex items-center gap-1 mb-1.5">
                  <span className="w-1 h-1 rounded-full bg-showcase-accent" />
                  <span className="text-[6.5px] sm:text-[8px] uppercase tracking-[0.14em] text-showcase-accent/80 font-semibold">
                    AI Agent
                  </span>
                </div>
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center bg-gradient-to-br from-showcase-accent to-[#62a6aa] shadow-[0_2px_6px_rgba(0,0,0,0.35)] mb-1.5">
                  <svg
              ...
              <div
                ...
                style={{
                  boxShadow:
                    "0 10px 24px -8px rgba(0,0,0,0.65), inset 0 1px 0 rgba(255,255,255,0.06)",
                  animation: "node-pop 0.6s ease-out 0.6s both",
                }}
              >
                {/* input handle */}
...
export function RoadmapAnimation() {
  ...
  return (
    <div className="flex-1 relative w-full h-full grid">
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === "ingest" && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0, transition: { duration: 0.5 } }}
            className="col-start-1 row-start-1 flex flex-col items-center justify-center w-full h-full z-10 absolute inset-0"
          >
            <div
              className="relative flex items-center justify-center w-full"
              style={{ height: "215px", perspective: "600px" }}
            >
              <div
                className="absolute left-1/2 top-[10px] z-20 flex flex-col items-center gap-2.5"
                style={{
                  animation:
                    "doc-ingest 4.0s cubic-bezier(0.45,0,0.2,1) forwards",
                ...
                >
                  ...
                  <div className="px-3 pb-3 pt-1 space-y-[7px]">
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      ...
                      <div className="h-[4px] flex-1 rounded-full bg-white/10 overflow-hidden">
                        <div className="h-full w-[98%] rounded-full bg-showcase-accent/70" />
                      </div>
                    </div>
                  </div>
                  <div
              ...
              {Array.from({ length: 24 }).map((_, i) => {
                ...
                const y =
                  Math.round((Math.sin(angle) * dist + (i % 3) * 25) * 1000) /
                  1000;
                return (
                  <div
                    key={`roadmap-particle-${i}`}
                    className="absolute w-[3.5px] h-[3.5px] sm:w-[4px] sm:h-[4px] rounded-full bg-showcase-accent z-30"
                    style={
                      {
                        left: "50%",
                        top: "calc(50% + 20px)",
                        ...
                        opacity: 0,
                        boxShadow: `0 0 8px 1.5px rgba(${SHOWCASE_ACCENT_RGB},0.8)`,
              ...
              <div
                className="absolute left-1/2 top-[calc(50%+20px)] -translate-x-1/2 -translate-y-1/2 w-40 h-40 bg-showcase-accent rounded-full blur-[40px] z-10 pointer-events-none"
                style={{
      ...
      <div
        className={`col-start-1 row-start-1 flex flex-col justify-center space-y-8 w-full h-full relative z-10 transition-all duration-500 ${phase === "roadmap" ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}
      >
        {/* Top nodes */}
        <div className="flex items-center justify-between w-full mx-auto relative px-4 sm:px-8">
          ...
          <div
            className={`absolute top-1/2 left-[36px] sm:left-[52px] right-1/2 h-[1px] bg-showcase-accent -translate-y-1/2 -z-10 origin-left transition-all duration-700 ${step >= 5 ? "scale-x-100 opacity-100" : "scale-x-0 opacity-0"}`}
          />

          {/* Progress Line W2 to W3 */}
          <div
            className={`absolute top-1/2 left-1/2 right-[36px] sm:right-[52px] h-[1px] bg-showcase-accent -translate-y-1/2 -z-10 origin-left transition-all duration-700 ${step >= 9 ? "scale-x-100 opacity-100" : "scale-x-0 opacity-0"}`}
          />
              ...
              >
                <div
                  className={`w-10 h-10 rounded-full border flex items-center justify-center text-xs transition-all duration-500 relative z-10 ${
                    isActive
                      ? "border-showcase-accent bg-[#111111] text-showcase-accent font-semibold scale-110 shadow-[0_0_15px_theme(colors.showcase-accent/30%)]"
                ...
                <span
                  className={`text-[10px] font-light transition-colors duration-500 ${isActive ? "text-white/80" : "text-white/70"}`}
                >
                  {wave.name}
                </span>
              </div>
        ...
        <div className="relative mx-auto w-full max-w-md px-1 sm:px-0">
          <div
            aria-hidden
            className="roadmap-glow absolute -inset-4 -z-10 rounded-3xl blur-xl"
            style={{
              background:
                `radial-gradient(circle at 50% 38%, rgba(${SHOWCASE_ACCENT_RGB},0.18), transparent 70%)`,
            }}
          />
          <SpotlightCard className="p-4 sm:p-6 w-full space-y-4 shadow-lg transition-all duration-500">
            <div
              key={currentWaveIdx}
              className="roadmap-title-in text-[13.5px] text-white uppercase tracking-widest font-light"
              style={{ fontFamily: "'Doto', 'Courier New', monospace" }}
            >
              {currentData.title}
            </div>
            <div className="space-y-3 relative overflow-hidden">
              {currentData.tasks.map((text, i) => {
                ...
                return (
                  <div
                    key={`${currentWaveIdx}-${i}`}
                    className="flex items-center gap-4 text-xs font-light transition-all duration-500 animate-fade-in-up"
                    style={{ animationDelay: `${i * 0.1}s` }}
                  >
                    <span
                      className={`w-5 h-5 rounded border flex items-center justify-center shrink-0 transition-colors duration-500 ${
                        isDone
                          ? "border-showcase-accent bg-showcase-accent/10"
                          : "border-white/10"
                      }`}
                    >
                      {isDone && (
                        <svg
                          key={`chk-${currentWaveIdx}-${i}`}
                          className="roadmap-check-draw"
                          width="11"
                          height="11"
                          viewBox="0 0 24 24"
                          fill="none"
                    ...
                    <span
                      className={`transition-all duration-500 ${isDone ? "line-through text-white/60" : "text-white/80"}`}
                    >
                      {text}
                    </span>
                  </div>
...
function IntroAnimation() {
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full relative flex flex-col justify-center items-center text-center p-8"
    >
      {/* Agent Label */}
      <div
        className={`transition-all duration-1000 ease-out absolute top-12 ${step >= 1 ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
      >
        <img
      ...
      <div className="relative h-24 w-full flex items-center justify-center">
        <div
          className={`absolute inset-0 flex items-center justify-center text-xl md:text-2xl font-light text-white transition-all duration-700 ease-in-out ${getLineClasses(2)}`}
        >
          Hi.
        </div>

        <div
          className={`absolute inset-0 flex items-center justify-center text-xl md:text-2xl font-light text-white transition-all duration-700 ease-in-out ${getLineClasses(3)}`}
        >
        ...
        <div
          className={`absolute inset-0 flex items-center justify-center text-xl md:text-2xl font-light text-white transition-all duration-700 ease-in-out ${getLineClasses(5)}`}
        >
        ...
        >
          ...
          <div className="flex flex-col items-center space-y-[-14px]">
            <svg
              className="w-12 h-6 text-[#dfe2d8] animate-chevron-1"
              viewBox="0 0 48 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
            ...
            <svg
              className="w-12 h-6 text-[#dfe2d8] animate-chevron-2"
              viewBox="0 0 48 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
            ...
            <svg
              className="w-12 h-6 text-[#dfe2d8] animate-chevron-3"
              viewBox="0 0 48 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
...
export function BlueprintAnimation() {
  const [phase, setPhase] = useState<
    "intro" | "import" | "generate" | "blueprint" | "complete"
  >("intro");
  const timerRefs = useRef<ReturnType<typeof setTimeout>[]>([]);
  ...
  return (
    <div
      className="flex-1 flex flex-col justify-center w-full opacity-0 animate-fade-in-up"
    ...
    >
      ...
      <div className="w-full relative min-h-[320px] grid overflow-hidden">
        ...
        <div
          className={`col-start-1 row-start-1 flex flex-col items-center justify-center transition-all duration-500 w-full h-full z-10 absolute inset-0 ${phase === "blueprint" || phase === "complete" ? "opacity-0 scale-95 pointer-events-none" : "opacity-100 scale-100"}`}
        >
          <AnimatePresence mode="wait">
            {phase === "intro" && (
              <motion.div
            ...
            {phase === "import" && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0, transition: { duration: 0.5 } }}
                className="flex flex-col items-center justify-center w-full absolute inset-0"
              >
                {/* Ingest stage: document glides down and shatters */}
                <div
                  className="relative flex items-center justify-center w-full h-[180px] sm:h-[215px]"
                ...
                >
                  ...
                  {Array.from({ length: 24 }).map((_, i) => {
                    ...
                    const y =
                      Math.round(
                        (Math.sin(angle) * dist + (i % 3) * 25) * 1000,
                      ) / 1000;
                    return (
                      <div
                        key={`particle-${i}`}
                  ...
                  <div
                    ...
                    style={{
                      background:
                        `linear-gradient(to bottom, rgba(${SHOWCASE_ACCENT_RGB},0), ${SHOWCASE_ACCENT}, ${SHOWCASE_ACCENT}, rgba(${SHOWCASE_ACCENT_RGB},0))`,
                      boxShadow:
                        `0 0 20px 4px rgba(${SHOWCASE_ACCENT_RGB},0.8), 0 0 40px rgba(${SHOWCASE_ACCENT_RGB},0.4)`,
                      animation:
                        "data-stream-flow 0.8s cubic-bezier(0.4, 0, 0.2, 1) forwards",
                ...
                <div className="flex flex-col items-center gap-4 z-10 relative mt-2 w-full max-w-[420px]">
                  ...
                  <div className="flex flex-wrap justify-center gap-3 sm:gap-4 w-full">
                    ...
                    <div
                      className="bg-showcase-accent/10 border border-showcase-accent/30 rounded-md px-3 py-1.5 sm:px-4 sm:py-2 text-[10px] sm:text-sm text-showcase-accent shadow-[0_0_15px_theme(colors.showcase-accent/15%)] font-medium whitespace-nowrap opacity-0 animate-[pop-in-pill_0.6s_ease-out_forwards]"
          ...
          {phase === "generate" && (
            <div className="flex flex-col items-center justify-center gap-6 animate-fade-in-up">
              <div className="relative w-28 h-28 sm:w-36 sm:h-36 flex items-center justify-center">
                <div className="absolute inset-[-10px] border border-showcase-accent/20 rounded-full premium-ping" />
                <div className="absolute inset-0 bg-showcase-accent/5 rounded-full blur-2xl" />
                <div className="w-full h-full flex items-center justify-center z-10">
                  <LabFlaskCanvas />
                </div>
              </div>
              <div className="flex flex-col items-center">
                <span className="text-sm sm:text-base text-showcase-accent uppercase tracking-widest font-medium">
                  Brewing Blueprint
                </span>
                <div className="w-32 sm:w-48 h-1 bg-white/10 rounded-full mt-4 overflow-hidden relative">
                  <div className="h-full bg-showcase-accent rounded-full animate-[loading-bar_3.5s_ease-in-out_forwards]" />
                </div>
              </div>
            </div>
          )}
        ...
        {/* Blueprint Layout */}
        <div
          className={`col-start-1 row-start-1 flex flex-col justify-center w-full h-full z-10 transition-opacity duration-300 ${phase === "blueprint" ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
        >
          <div
          ...
          >
            <span className="text-[10px] sm:text-xs text-showcase-accent uppercase tracking-[0.1em] font-medium mb-1 drop-shadow-[0_0_8px_theme(colors.showcase-accent/50%)]">
              Tailored Blueprint
            </span>
            <span className="text-lg sm:text-xl text-white font-light text-center">
              Ops Scaling Architecture
            </span>
          </div>

          {/* Contextual Mapping */}
          <div className="flex flex-col gap-2.5 sm:gap-4 w-full max-w-full sm:max-w-[440px] mx-auto">
            <style>{`
            ...
            <div
              ...
              style={{
                ...
                animationDelay: phase === "blueprint" ? "0.2s" : "0s",
              }}
            >
              <div className="absolute left-0 top-3 bottom-3 w-[3px] rounded-full bg-white/20" />
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-showcase-accent/10 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000" />
              {/* left */}
              ...
              <div className="flex w-[16px] sm:w-[50px] shrink-0 h-3 z-10 items-center mx-0.5 sm:mx-2">
                <div className="relative h-[1px] w-full bg-white/20">
                  ...
                  <div className="absolute left-1/2 top-[0.5px] -translate-x-1/2 -translate-y-1/2 flex items-center justify-center">
                    <div className="absolute w-[18px] h-[18px] rounded-full border border-showcase-accent/40 animate-[ping_2.5s_cubic-bezier(0,0,0.2,1)_infinite] opacity-60" />
                    <div className="absolute w-[10px] h-[10px] rounded-full border border-showcase-accent/60 bg-[#1c1c22]/80 backdrop-blur-md shadow-[0_0_6px_rgba(178,204,162,0.3)]" />
                    <div className="w-[4px] h-[4px] rounded-full bg-showcase-accent shadow-[0_0_8px_1px_rgba(178,204,162,1)] z-10" />
                  </div>
                </div>
              </div>
              {/* right */}
              <div className="flex items-center gap-2 sm:gap-2.5 flex-1 min-w-0 pl-1 sm:pl-4 justify-end text-right relative z-10">
                <div className="flex flex-col min-w-0 w-full text-right">
                  <span className="text-[7.5px] sm:text-[10px] text-showcase-accent/80 uppercase tracking-wider mb-0.5">
                    Resolution Layer
                  </span>
                  <span className="text-[9px] sm:text-sm text-white/90 font-medium leading-tight sm:leading-snug truncate">
                ...
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-showcase-accent/12 border border-showcase-accent/35 flex items-center justify-center text-showcase-accent shrink-0">
                  <svg
                    className="w-3.5 h-3.5"
                    viewBox="0 0 24 24"
                    fill="none"
            ...
            <div
              ...
              style={{
                boxShadow:
                  "0 12px 30px -14px rgba(0,0,0,0.65), inset 0 1px 0 rgba(255,255,255,0.06)",
                animationDelay: phase === "blueprint" ? "0.4s" : "0s",
              }}
            >
              <div className="absolute left-0 top-3 bottom-3 w-[3px] rounded-full bg-white/20" />
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-showcase-accent/10 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000" />
              ...
              <div className="flex w-[16px] sm:w-[50px] shrink-0 h-3 z-10 items-center mx-0.5 sm:mx-2">
                <div className="relative h-[1px] w-full bg-white/20">
                  ...
                  <div className="absolute left-1/2 top-[0.5px] -translate-x-1/2 -translate-y-1/2 flex items-center justify-center transform scale-75 sm:scale-100">
                    <div
                      className="absolute w-[18px] h-[18px] rounded-full border border-showcase-accent/40 animate-[ping_2.5s_cubic-bezier(0,0,0.2,1)_infinite] opacity-60"
          ...
          >
            <span className="text-[9px] sm:text-[10px] text-showcase-accent/60 uppercase tracking-[0.15em] font-medium drop-shadow-sm">
        ...
        <AnimatePresence>
          <motion.div
            ...
            exit={{ opacity: 0, transition: { duration: 0.5 } }}
            className={`col-start-1 row-start-1 flex flex-col justify-center items-center p-4 w-full h-full transition-all duration-500 delay-200 ${phase === "complete" ? "scale-100" : "scale-105 pointer-events-none"}`}
          >
            {phase === "complete" && (
              <div className="flex flex-col items-center gap-3 animate-fade-in-up">
                <div className="relative flex flex-col items-center gap-3 z-20">
                  ...
                  <span className="text-[10px] sm:text-[11px] text-showcase-accent/90 uppercase tracking-[0.22em] font-medium px-4 py-1.5 bg-[#0d0d0d]/80 border border-white/5 rounded-full whitespace-nowrap backdrop-blur-sm shadow-[0_4px_12px_rgba(0,0,0,0.5)]">
                    System Blueprint
                  </span>
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      h4Refs.current.forEach((ref, idx) => {
        ...
        if (h4CenterY > vh * 0.12 && h4CenterY < vh * 0.7) {
          newlyConnected = idx;
        }
      });

      setConnectedIndex((prev) =>
        prev !== newlyConnected ? newlyConnected : prev,
      );
    };
              ...
              <div
                ...
                className={`flex flex-col justify-center min-h-[50vh] transition-all duration-500 ${
                  activeIndex === idx
                    ...
                    : "opacity-30 scale-95 lg:opacity-20"
                }`}
              >
                ...
                <div className="flex items-center mb-6 relative w-full lg:w-[calc(100%+4rem)] z-10">
                  ...
                  >
                    {/* The Line */}
                    <div className="h-[1px] bg-gradient-to-r from-transparent via-showcase-accent/50 to-showcase-accent/80 flex-grow" />

                    {/* Premium Node Connector */}
                    <div className="relative flex items-center justify-center shrink-0 translate-x-[1px]">
                      {/* Outer ping animation */}
                      <div className="absolute w-[24px] h-[24px] rounded-full border-[1.5px] border-showcase-accent/40 animate-[ping_2.5s_cubic-bezier(0,0,0.2,1)_infinite] opacity-60" />
                      {/* Static Glass Ring */}
                      <div className="absolute w-[14px] h-[14px] rounded-full border border-showcase-accent/60 bg-[#1c1c22]/80 backdrop-blur-md shadow-[0_0_8px_rgba(178,204,162,0.3)]" />
                      {/* Solid glowing core */}
                      <div className="w-[6px] h-[6px] rounded-full bg-showcase-accent shadow-[0_0_12px_2px_rgba(178,204,162,1)] z-10" />
                    </div>
                  </div>
                </div>
                ...
                <SpotlightCard
                  className={`flex lg:hidden w-full mt-10`}
                  style={{ minHeight: "380px" }}
                >
                  <div className="absolute inset-0 p-3 sm:p-4 md:p-6 flex flex-col overflow-hidden">
                    {idx === 0 && (
                    ...
                    {idx === 4 && (
                      <div className="w-full flex flex-col gap-4 h-full justify-center relative">
                        <WorkflowAnimation />
                      </div>
                    )}
                  </div>
          ...
          <div className="hidden lg:col-span-7 lg:sticky lg:top-[12vh] lg:flex items-center justify-center z-20 mx-auto w-full max-w-[850px] aspect-[4/3] lg:aspect-[16/11]">
            ...
            >
              ...
              <div className="flex-1 relative w-full h-full">
                ...
                >
                  <div
                    key={`diag-${activeIndex === 0 ? "active" : "inactive"}`}
                ...
                >
                  <div
                    key={`blue-${activeIndex === 1 ? "active" : "inactive"}`}
                    className="flex-1 relative w-full h-full"
                  >
                    {activeIndex === 1 && <BlueprintAnimation />}
                  </div>
                  <div className="text-[10px] text-white/50 text-center uppercase tracking-widest mt-6 pb-6 font-light">
                    * Schematic mapping how databases interface with automation
                    layers.
                  </div>
                </div>
                ...
                >
                  <div
                    key={`road-${activeIndex === 2 ? "active" : "inactive"}`}
                  ...
                  <div className="text-[10px] text-[#dfe2d8] text-center uppercase tracking-widest mt-6 pb-6 font-manrope font-light">
                    * Phased wave system mapping out operational integrations
                    step-by-step.
                  </div>
                </div>
                ...
                >
                  <div
                    key={`chat-${activeIndex === 3 ? "active" : "inactive"}`}
                    className="flex-1 w-full h-full relative"
                  >
                    {activeIndex === 3 && <ConsoleAnimation />}
                  </div>
                  <div className="text-[10px] text-white/50 text-center uppercase tracking-widest mt-6 pb-6 font-light">
                    * Conversational interface for strategic operational
                    insights.
                  </div>
                </div>
                ...
                >
                  <div
                    key={`wf-${activeIndex === 4 ? "active" : "inactive"}`}
                    className="w-full flex flex-col gap-4 h-full justify-center relative"
                  >
